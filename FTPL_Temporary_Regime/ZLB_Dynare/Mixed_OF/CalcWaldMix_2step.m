...
[M_Orthodox_Pre, oo_Orthodox_Pre] = driver_function_orthodox(coefPre,rho_estPre);
warning off
[M_Orthodox_Pre_ZLB, oo_Orthodox_Pre_ZLB] = driver_function_orthodox_zlb(coefPre,rho_estPre);
...
[residualPre,residualPre_dt, innoPre,rho_estPre,nst_inxPre] = GetRes_LIMLPre(M_Orthodox_Pre,act_dataPre,inx_expect,inx_eqs);   % LIML
save resid_innoPre residualPre residualPre_dt innoPre
save rho_estPre rho_estPre
[residualPre_ZLB,residualPre_dt_ZLB, innoPre_ZLB,rho_estPre_ZLB,nst_inxPre_ZLB] = GetRes_LIMLPre_ZLB(M_Orthodox_Pre_ZLB,act_dataPre,inx_expect,inx_eqs);   % LIML
...
[M_Orthodox_Pre, oo_Orthodox_Pre] = driver_function_orthodox(coefPre,rho_estPre);
[M_Orthodox_Pre_ZLB, oo_Orthodox_Pre_ZLB] = driver_function_orthodox_zlb(coefPre,rho_estPre);
nvarsPre=size(oo_Orthodox_Pre.dr.ghx,1);
warning on
...
% dynare orthodox.mod noclearall nopreprocessoroutput notime;    
[M_FTPL_Post, oo_FTPL_Post] = driver_function_ftpl(coefPost,rho_estPost);
warning off
[M_FTPL_Post_ZLB, oo_FTPL_Post_ZLB] = driver_function_ftpl_zlb(coefPost,rho_estPost);
...
[residualPost,residualPost_dt, innoPost,rho_estPost,nst_inxPost] = GetRes_LIMLPost(M_FTPL_Post,act_dataPost,inx_expect,inx_eqs);   % LIML
save resid_innoPost residualPost residualPost_dt innoPost
save rho_estPost rho_estPost
[residualPost_ZLB,residualPost_dt_ZLB, innoPost_ZLB,rho_estPost_ZLB,nst_inxPost_ZLB] = GetRes_LIMLPost_ZLB(M_FTPL_Post_ZLB,act_dataPost,inx_expect,inx_eqs);   % LIML
...
[M_FTPL_Post, oo_FTPL_Post] = driver_function_ftpl(coefPost,rho_estPost);
[M_FTPL_Post_ZLB, oo_FTPL_Post_ZLB] = driver_function_ftpl_zlb(coefPost,rho_estPost);
nvarsPost=size(oo_FTPL_Post.dr.ghx,1);
warning on
...
[boots_dataPre, NKweight_output, NBsimPre] = Boots_data_endog_switching_parallel_orthodox_ZLB_NB(act_dataPre,nvarsPre,innoPre,innoPre_ZLB,nboot,start_lagged_pinf,coefPre,rho_estPre,rho_estPre_ZLB);

% load surplus.mat
[boots_data, NKweight_output, NB_Tall] = Boots_data_endog_switching_parallel_ftpl_ZLB(act_dataPost,boots_dataPre,NBsimPre,nvarsPost,innoPost,innoPost_ZLB,nboot,start_lagged_pinf,coefPost,rho_estPost,rho_estPost_ZLB,NBbase);
% [boots_data, NKweight_output, NB_Tall] = Boots_data_endog_switching_parallel_ZLB_mix(act_dataPre,act_dataPost,nvarsPre,nvarsPost,innoPre,innoPost,innoPre_ZLB,innoPost_ZLB,nboot,start_lagged_pinf,coefPre,coefPost,rho_estPre,rho_estPost,rho_estPre_ZLB,rho_estPost_ZLB,NBbase);
%%   
% Step3:
...
[pvalue_novar, Wald_novar, Trans_Wald_novar, mmetric_novar all_coeffs_novar, act_coeffs_novar, mean_var_sims_novar] = Wald_nonstationary(act_dataAll(var_no,3:end),boots_data(var_no,:,:),act_nonstatResid(1,3:end),boots_nonstatResid,rho_nonstat,var_order,0);
