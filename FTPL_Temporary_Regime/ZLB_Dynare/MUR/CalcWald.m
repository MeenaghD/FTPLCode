...
[M_, oo_] = driver_function_orthodox_weighted(coef,rho_est);
warning off
[M_ZLB, oo_ZLB] = driver_function_orthodox_weighted_zlb(coef,rho_est);
...
[residual_ZLB,residual_dt_ZLB, inno_ZLB,rho_est_ZLB,nst_inx_ZLB] = GetRes_LIML_ZLB(M_ZLB,act_data,inx_expect,inx_eqs);   % LIML
% Surplus is an ARIMA(1,1,0), do this separately
rho_est_ZLB(13)=rho_surp;
inno_ZLB(12,:)=[NaN surp_inno];
save resid_inno_ZLB residual_ZLB residual_dt_ZLB inno_ZLB
save rho_est_ZLB rho_est_ZLB
warning off
% dynare orthodox.mod noclearall nopreprocessoroutput notime;    
[M_, oo_] = driver_function_orthodox_weighted(coef,rho_est);
[M_ZLB, oo_ZLB] = driver_function_orthodox_weighted_zlb(coef,rho_est_ZLB);
warning on
...
warning off
[boots_data, NKweight_output,nsims,NB_Tall,debtgdp2_all,all_coef25] = Boots_data_endog_switching_parallel_ZLB_orthodox_weighted(act_data,nvars,inno,inno_ZLB,nboot,start_lagged_pinf,coef,rho_est,rho_est_ZLB,NBbase,nomgdp);
...
act_nonstatResid=residual(nst_inx,3:end);
...
[pvalue_novar, Wald_novar, Trans_Wald_novar, all_coeffs_novar, act_coeffs_novar, mean_var_sims_novar] = Wald_nonstationary(act_data(var_no,3:end),boots_data(var_no,:,:),act_nonstatResid,boots_nonstatResid,rho_nonstat,var_order,0);
