function [ YY, NKweight_output,nsims, NB_Tall] = Boots_data_endog_switching_parallel_ftpl(act_data,nvars,inno,nboot,start_lagged_pinf,coef,rho_est,NBbase)
%Input:
...
% Create a reproducible random stream unique to this iteration
    ...
    for t = 3:T 
        ...
        for t1 = t:T+49
            if t1==t
                surplus(1,t1)=surplus(1,t1-1)+rho_est_local(13)*(surplus(1,t1-1)-surplus(1,t1-2))+inno(12, inx(t));
            else
        ...
        pinfbar=ones(1,size(inno,2)).*P_T/49;
        inno(13,:)=pinfbar;
